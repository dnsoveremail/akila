...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 AKILA
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'akila_9000-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appAkila.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appAkila.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
