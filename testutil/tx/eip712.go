...
func PrepareEIP712CosmosTx(
	...
	accNumber := appAkila.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appAkila.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		akilaCodec := codec.NewProtoCodec(registry)
