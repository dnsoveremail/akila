...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aakila amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	res, err := DeliverEthTx(akilaApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, akilaApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := akilaApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := akilaApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := akilaApp.EvmKeeper.GetNonce(ctx, from)
