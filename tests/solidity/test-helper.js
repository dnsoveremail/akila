...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network akila', 'run all tests using akila network')
    .example(
      '$0 --network akila --allowTests=test1,test2',
      'run only test1 and test2 using akila network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|akila')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print akilad output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'akila' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or akila')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-akila']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-akila'
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'akilad started'

    const akiladProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting akilad process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      akiladProc.stdout.pipe(process.stdout)
    }

    akiladProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start akilad timeout!')), timeout)
  })
