...
HTTPS_GIT := https://github.com/akila/akila.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=akila \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(AKILA_BINARY) \
...
build-docker:
	...
	mkdir -p ./build/.akilad
	echo '#!/usr/bin/env bash' > ./build/akilad
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/akilad
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/akilad
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.akilad:/home/akila/.akilad $$IMAGE_NAME akilad "$$@"' >> ./build/akilad
	chmod +x ./build/akilad
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/akila/akila"
	godoc -http=:6060
